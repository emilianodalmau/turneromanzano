...
export default function LicenseScheduleConfigPage() {
  ...
  const scheduleRef = useMemoFirebase(
    () => (firestore && user ? doc(firestore, 'licenseScheduleConfigurations', 'default') : null),
    [firestore, user]
  );

  const { data: scheduleConfig, isLoading: isDocLoading } = useDoc<LicenseScheduleConfiguration>(scheduleRef);
